...
const runEvent = (topic, message, packet) => {
...
const unRegisterEvent$1 = async (topic, vm = "none") => {
  const listenerList = messageListenerMap.get(topic);
  if (listenerList && listenerList.length) {
    ...
    if (indexArray.length) {
      ...
      if (listenerList.length > 0)
        messageListenerMap.set(topic, listenerList);
      else
        messageListenerMap.delete(topic);
    }
  }
};
const registerEvent$1 = async (topic, callback, vm = "none") => {
  await unRegisterEvent$1(topic, vm);
  messageListenerMap.has(topic) || messageListenerMap.set(topic, []);
  messageListenerMap.get(topic).push({ callback, vm });
};
const clearEvent$1 = async () => {
...
let client = null;
const publishBuffer = [];
const subscribeBuffer = [];
const isConnected = () => {
  if (client == null ? void 0 : client.connected)
    return true;
  return false;
};
const subscribe = async (topicArray, qos = 1, opts = {
  qos: 1,
  nl: false,
  rap: false,
  rh: 0,
  ...
  if (!(client == null ? void 0 : client.connected)) {
    ...
    client == null ? void 0 : client.subscribe(topicArray, opts, callback);
  }
};
const unSubscribe = async (unTopic, opts = {}, callback) => {
  if (callback) {
    client == null ? void 0 : client.unsubscribe(unTopic, opts, callback);
  } else {
    client == null ? void 0 : client.unsubscribe(unTopic, opts, () => {
      common.debug().log(`unsubscribe: ${unTopic}`);
    });
  }
};
const publish = async (topic, message, qos = 0, opts = {
  qos: 0,
  retain: false,
  dup: false,
  properties: {}
...
const onMessage = async () => {
  client == null ? void 0 : client.on("message", (topic, message, packet) => {
    if (message)
      event.runEvent(topic, message, packet);
  });
};
const onConnect = async (url) => {
  client == null ? void 0 : client.on("connect", () => {
...
const onReconnect = async () => {
  client == null ? void 0 : client.on("reconnect", () => {
    var _a;
    common.debug().log("try to reconnect:", (_a = client == null ? void 0 : client.options) == null ? void 0 : _a.hostname);
    event.runEvent("on-reconnect", "");
  });
};
const onEnd = async () => {
  client == null ? void 0 : client.on("end", () => {
...
const reconnect = async (url, _options) => {
  disconnect();
  connect(url, _options);
  common.debug().log("mqtt reconnecting");
};
const unRegisterEvent = async (topic, vm = "none") => event.unRegisterEvent(topic, vm);
const registerEvent = async (topic, callback, vm = "none") => event.registerEvent(topic, callback, vm);
